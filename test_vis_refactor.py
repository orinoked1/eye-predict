...
try:
    tidy_data = pd.read_pickle(path + "/etp_data/processed/tidy_fixation_data_126_128.pkl")
except:
    both_eye_data = pd.read_csv(path + "/etp_data/processed/126_138_both_eye_data_fixations.csv") #(both_eye_data_path)
    one_eye_data = pd.read_csv(path + "/etp_data/processed/126_138_one_eye_data_fixations.csv") #(one_eye_data_path)
    all_data = pd.concat([both_eye_data, one_eye_data])
    tidy_data = data.data_tidying_for_fixation_dataset_building(all_data, cfg['exp']['etp']['screen_resolution'])
    tidy_data.to_pickle(path + "/etp_data/processed/tidy_fixation_data_126_128.pkl")
...
scanpath_df.to_pickle(path + "/etp_data/processed/scanpath_df_fixation_126_128.pkl")
